...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('m_branch', function (Blueprint $table) {
            ...
            $table->integer('labor_insurance_payment_method')->nullable()->comment('保険料率納付区分');
            $table->date('labor_insurance_establishment_date')->nullable()->comment('労働保険成立年月日');
            $table->string('insurance_office_no', 20)->nullable()->comment('健康保険組合・事業所番号');
            $table->string('insurance_office_reference_no', 20)->nullable()->comment('健康保険・事業所整理番号');
            $table->string('pension_office_no', 10)->nullable()->comment('厚生年金基金・事業所番号');
            $table->string('pension_office_reference_prefecture', 10)->nullable()->comment('事業所整理記号・都道府県');
            $table->string('pension_office_reference_no_cities', 10)->nullable()->comment('事業所整理記号・都市区記号');
            $table->string('pension_office_reference_no_office', 10)->nullable()->comment('事業所整理記号・事業所記号');
            $table->integer('pension_office_id')->nullable()->comment('年金事務所');
            $table->string('employment_insurance_office_no', 20)->nullable()->comment('雇用保険・事業者番号');
            $table->date('employment_insurance_establishment_date')->nullable()->comment('雇用保険設立年月日');
            $table->integer('hello_work_id')->nullable()->comment('公共職業安定所');
            $table->integer('labor_bureau_id')->nullable()->comment('労働局');
            $table->integer('labor_supervision_id')->nullable()->comment('労働基準監督署');
