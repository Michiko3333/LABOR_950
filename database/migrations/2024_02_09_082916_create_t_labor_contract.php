...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('t_labor_contract', function (Blueprint $table) {
            ...
            $table->date('contract_start_date')->comment('契約開始年月日');
            $table->date('contract_end_date')->nullable()->comment('契約終了年月日');
            $table->integer('1month_before_basic_days')->nullable()->comment('1ヶ月前基礎日数');
            $table->integer('2month_before_basic_days')->nullable()->comment('2ヶ月前基礎日数');
            $table->integer('3month_before_basic_days')->nullable()->comment('3ヶ月前基礎日数');
            ...
            $table->string('break_time')->nullable()->comment('休憩時間');
            $table->string('holiday_regulations')->nullable()->comment('休日規定');
            $table->tinyInteger('overtime_work_flg')->nullable()->comment('時間外労働フラグ');
            $table->tinyInteger('holiday_work_flg')->nullable()->comment('休日出勤フラグ');
            $table->string('matters_of_retirement')->nullable()->comment('退職に関する事項');
            $table->string('other_contract_matters', 800)->nullable()->comment('その他の契約事項');
