...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('m_pension_office', function (Blueprint $table) {
            ...
            $table->string('system_name_e', 255)->nullable()->comment('提出先名称E');
            $table->string('system_union_name_e', 255)->nullable()->comment('提出先名称E（県名込み）');
            $table->string('identifier_f', 255)->nullable()->comment('提出先識別子F');
            $table->string('system_name_f', 255)->nullable()->comment('提出先名称F');
            $table->string('system_union_name_f', 255)->nullable()->comment('提出先名称F（県名込み）');
            $table->string('identifier_g', 255)->nullable()->comment('提出先識別子G');
            $table->string('system_name_g', 255)->nullable()->comment('提出先名称G');
            $table->string('system_union_name_g', 255)->nullable()->comment('提出先名称G（県名込み）');
            $table->string('identifier_m', 255)->nullable()->comment('提出先識別子M');
            $table->string('system_name_m', 255)->nullable()->comment('提出先名称M');
            $table->string('system_union_name_m', 255)->nullable()->comment('提出先名称M（県名込み）');
