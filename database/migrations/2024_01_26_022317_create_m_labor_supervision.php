...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('m_labor_supervision', function (Blueprint $table) {
            ...
            $table->string('submit_name_hij', 255)->nullable()->comment('提出先名称HIJ');
            $table->string('submit_union_name_hij', 255)->nullable()->comment('提出先名称HIJ（県名込み）');
            $table->string('identifier_l', 255)->nullable()->comment('提出先識別子L');
            $table->string('submit_name_l', 255)->nullable()->comment('提出先名称L');
            $table->string('submit_union_name_l', 255)->nullable()->comment('提出先名称L（県名込み）');
            $table->string('identifier_n', 255)->nullable()->comment('提出先識別子N');
            $table->string('submit_name_n', 255)->nullable()->comment('提出先名称N');
            $table->string('submit_union_name_n', 255)->nullable()->comment('提出先名称N（県名込み）');
