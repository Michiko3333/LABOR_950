...
class HealthInsuranceDependentChangeRequest extends BaseRequest
{
    ...
    public static function rules(): array
    {
        ...
        return [
            ...
            "company_name" => 'string|max:25|regex:/^[ぁ-んァ-ヴ０-９ー一-龥々ａ-ｚＡ-Ｚ　0-9a-zA-Z ]+\z/u',
            "headquarters_address" => ['required', 'string', 'max:50', new FullwidthAndMiscellaneousChars(true)],
            ...
            "spouse_mynumber_card_no" => 'nullable|string|max:12|regex:/^[0-9]{10,12}$/u',
            ...
            "spouse_reason_type_2" => 'nullable|string|in:配偶者の就職,婚姻,離職,収入減少,その他',
            "spouse_reason_2" => 'nullable|string|max:255',
            "spouse_reason_type" => 'nullable|string|in:死亡,離婚,就職・収入増加,75歳到達,障害認定,その他',
            ...
            "eligibility_confirmation_letter_1" => 'nullable|int|in:1',
            ...
            "other_dependent1_mynumber_card_no" => 'nullable|string|max:12|regex:/^[0-9]{10,12}$/u',
            ...
            "other_dependent1_reason_type" => 'nullable|string|in:出生,離職,収入減少,同居,その他',
            "other_dependent1_reason_2" => 'nullable|string|max:255',
            ...
            "other_dependent1_reason_type_2" => 'nullable|string|in:死亡,離婚,就職,収入増加,75歳到達,障害認定,その他',
            ...
            "eligibility_confirmation_letter_2" => 'nullable|int|in:1',
            ...
            "other_dependent2_mynumber_card_no" => 'nullable|string|max:12|regex:/^[0-9]{10,12}$/u',
            ...
            "other_dependent2_reason_type" => 'nullable|string|in:出生,離職,収入減少,同居,その他',
            ...
            "other_dependent2_reason_type_2" => 'nullable|string|in:死亡,離婚,就職,収入増加,75歳到達,障害認定,その他',
            "other_dependent2_reason_2" => 'nullable|string|max:255',
            ...
            "eligibility_confirmation_letter_3" => 'nullable|int|in:1',
        ...
        $validator->after(function ($validator) {
            ...
            if(!empty($accepted_month) && !empty($accepted_day) && !empty($accepted_year)) {
                if(ctype_digit($accepted_month) && ctype_digit($accepted_day)) {
                    ...
                    if (Carbon::create($accepted_year,$accepted_month, $accepted_day)->gt($today)) {
                        $validator->errors()->add('accepted_day','1枚目_3_事業主等受付年月日に未来の日付を入力しないでください。');
                    }
                }
            }
            ...
            if (!empty($birthday_month) && !empty($birthday_day) && !empty($birthday_year)) {
                if (ctype_digit($birthday_month) && ctype_digit($birthday_day)) {
                    ...
                    if (Carbon::create($birthday_year,$birthday_month, $birthday_day)->gt($today)) {
                        $validator->errors()->add('birthday_day','1枚目_7_生年月日に未来の日付を入力しないでください。');
                    }
                }
            }
            if ($birthday_era === '5') {
            ...
            if (!empty($notification_month) && !empty($notification_day) && !empty($notification_year)) {
                if (ctype_digit($notification_month) && ctype_digit($notification_day)) {
                    ...
                    if (Carbon::create($notification_year,$notification_month, $notification_day)->gt($today)) {
                        $validator->errors()->add('notification_day','1枚目_13_氏名_日付に未来の日付を入力しないでください。');
                    }
                }
            }
            ...
            if (!empty($spouse_birthday_month) && !empty($spouse_birthday_day) && !empty($spouse_birthday_year)) {
                if (ctype_digit($spouse_birthday_month) && ctype_digit($spouse_birthday_day)) {
                    ...
                    if (Carbon::create($spouse_birthday_year,$spouse_birthday_month, $spouse_birthday_day)->gt($today)) {
                        $validator->errors()->add('spouse_birthday_day','1枚目_14_生年月日に未来の日付を入力しないでください。');
                    }
                }
            }
            if ($spouse_birthday_era === '5') {
            ...
            if (!empty($acquisition_month) && !empty($acquisition_day) && !empty($acquisition_year)) {
                if (ctype_digit($acquisition_month) && ctype_digit($acquisition_day)) {
                    ...
                    if (Carbon::create($acquisition_year,$acquisition_month, $acquisition_day)->gt($today)) {
                        $validator->errors()->add('acquisition_day','1枚目_10_取得年月日に未来の日付を入力しないでください。');
                    }
                }
            }
            if ($acquisition_era === '5') {
            ...
            if (!empty($spouse_become_date_month) && !empty($spouse_become_date_day) && !empty($spouse_become_date_year)) {
                if (ctype_digit($spouse_become_date_month) && ctype_digit($spouse_become_date_day)) {
                    if (!checkdate($spouse_become_date_month, $spouse_become_date_day, $spouse_become_date_year)) {
                        $validator->errors()->add('spouse_become_date_day', '1枚目_21_被扶養者になった日は正しい日付を入力してください。');
                    }
                    if (Carbon::create($spouse_become_date_year,$spouse_become_date_month, $spouse_become_date_day)->gt($today)) {
                        $validator->errors()->add('spouse_become_date_day','1枚目_21_被扶養者に未来の日付を入力しないでください。');
                    }
                }
            }
            if($spouse_become_date_era === '7') {
            ...
            if (!empty($spouse_become_date_era) && !empty($spouse_become_date_year) && !empty($spouse_become_date_month) && !empty($spouse_become_date_day)
            && !empty($spouse_remove_date_era) && !empty($spouse_remove_date_year) && !empty($spouse_remove_date_month) && !empty($spouse_remove_date_day)) {
                if ($spouse_remove_date_year < $spouse_become_date_year) {
                    $validator->errors()->add('spouse_remove_date_day', '1枚目_25_被扶養者でなくなった日は1枚目_21_被扶養者になった日以降を入力してください。');
                } elseif ($spouse_remove_date_year == $spouse_become_date_year) {
                    if ($spouse_remove_date_month < $spouse_become_date_month) {
                        $validator->errors()->add('spouse_remove_date_day', '1枚目_25_被扶養者でなくなった日は1枚目_21_被扶養者になった日以降を入力してください。');
                    } elseif ($spouse_remove_date_month == $spouse_become_date_month) {
                        if ($spouse_remove_date_day < $spouse_become_date_day) {
                            $validator->errors()->add('spouse_remove_date_day', '1枚目_25_被扶養者でなくなった日は1枚目_21_被扶養者になった日以降を入力してください。');
                        }
                    }
                }
            ...
            if (!empty($spouse_remove_date_month) && !empty($spouse_remove_date_day) && !empty($spouse_remove_date_year)) {
                if (ctype_digit($spouse_remove_date_month) && ctype_digit($spouse_remove_date_day)) {
                    if (!checkdate($spouse_remove_date_month, $spouse_remove_date_day, $spouse_remove_date_year)) {
                        $validator->errors()->add('spouse_remove_date_day', '1枚目_25_被扶養者でなくなった日は正しい日付を入力してください。');
                    }
                    if (Carbon::create($spouse_remove_date_year,$spouse_remove_date_month, $spouse_remove_date_day)->gt($today)) {
                        $validator->errors()->add('spouse_remove_date_day','1枚目_25_被扶養者でなくなった日に未来の日付を入力しないでください。');
                    }
                }
            }
            if($spouse_remove_date_era === '7') {
            ...
            if (!empty($spouse_passed_away_date_month) && !empty($spouse_passed_away_date_day) && !empty($spouse_passed_away_date_year)) {
                if (ctype_digit($spouse_passed_away_date_month) && ctype_digit($spouse_passed_away_date_day)) {
                    if (!checkdate($spouse_passed_away_date_month, $spouse_passed_away_date_day, $spouse_passed_away_date_year)) {
                        $validator->errors()->add('spouse_passed_away_date_day', '1枚目_26_理由_死亡年月日は正しい日付を入力してください。');
                    }
                    if (Carbon::create($spouse_passed_away_date_year,$spouse_passed_away_date_month, $spouse_passed_away_date_day)->gt($today)) {
                        $validator->errors()->add('spouse_passed_away_date_day','1枚目_26_理由_死亡年月日に未来の日付を入力しないでください。');
                    }
                }
            }
            ...
            if (!empty($spouse_special_requirements_applicable_date_month) && !empty($spouse_special_requirements_applicable_date_day) && !empty($spouse_special_requirements_applicable_date_year)) {
                if (ctype_digit($spouse_special_requirements_applicable_date_month) && ctype_digit($spouse_special_requirements_applicable_date_day)) {
                    ...
                    if (Carbon::create($spouse_special_requirements_applicable_date_year,$spouse_special_requirements_applicable_date_month, $spouse_special_requirements_applicable_date_day)->gt($today)) {
                        $validator->errors()->add('spouse_special_requirements_applicable_date_day','1枚目_27_海外特例要件に該当した日に未来の日付を入力しないでください。');
                    }
                }
            }
            ...
            if (!empty($spouse_special_requirements_non_applicable_date_month) && !empty($spouse_special_requirements_non_applicable_date_day) && !empty($spouse_special_requirements_non_applicable_date_year)) {
                if (ctype_digit($spouse_special_requirements_non_applicable_date_month) && ctype_digit($spouse_special_requirements_non_applicable_date_day)) {
                    ...
                    if (Carbon::create($spouse_special_requirements_non_applicable_date_year,$spouse_special_requirements_non_applicable_date_month, $spouse_special_requirements_non_applicable_date_day)->gt($today)) {
                        $validator->errors()->add('spouse_special_requirements_non_applicable_date_day','1枚目_29_海外特例要件に非該当となった日に未来の日付を入力しないでください。');
                    }
                }
            }
            ...
            if (!empty($spouse_special_requirements_applicable_date_year) && !empty($spouse_special_requirements_applicable_date_month) && !empty($spouse_special_requirements_applicable_date_day)
            && !empty($spouse_special_requirements_non_applicable_date_year) && !empty($spouse_special_requirements_non_applicable_date_month) && !empty($spouse_remove_date_day)) {
                if ($spouse_special_requirements_non_applicable_date_year < $spouse_special_requirements_applicable_date_year) {
                    $validator->errors()->add('spouse_special_requirements_non_applicable_date_day', '1枚目_27_海外特例要件に該当した日は1枚目_29_海外特例要件に非該当となった日以降を入力してください。');
                } elseif ($spouse_special_requirements_non_applicable_date_year == $spouse_special_requirements_applicable_date_year) {
                    if ($spouse_special_requirements_non_applicable_date_month < $spouse_special_requirements_applicable_date_month) {
                        $validator->errors()->add('spouse_special_requirements_non_applicable_date_day', '1枚目_27_海外特例要件に該当した日は1枚目_29_海外特例要件に非該当となった日以降を入力してください。');
                    } elseif ($spouse_special_requirements_non_applicable_date_month == $spouse_special_requirements_applicable_date_month) {
                        if ($spouse_special_requirements_non_applicable_date_day < $spouse_special_requirements_applicable_date_day) {
                            $validator->errors()->add('spouse_special_requirements_non_applicable_date_day', '1枚目_27_海外特例要件に該当した日は1枚目_29_海外特例要件に非該当となった日以降を入力してください。');
                        }
                    }
                }
            ...
            if (!empty($spouse_domestic_transfer_date_month) && !empty($spouse_domestic_transfer_date_day) && !empty($spouse_domestic_transfer_date_year)) {
                if (ctype_digit($spouse_domestic_transfer_date_month) && ctype_digit($spouse_domestic_transfer_date_day)) {
                    ...
                    if (Carbon::create($spouse_domestic_transfer_date_year,$spouse_domestic_transfer_date_month, $spouse_domestic_transfer_date_day)->gt($today)) {
                        $validator->errors()->add('spouse_domestic_transfer_date_day','1枚目_30_理由_国内転入日に未来の日付を入力しないでください。');
                    }
                }
            }
            ...
            if (!empty($other_dependent1_birthday_month) && !empty($other_dependent1_birthday_day) && !empty($other_dependent1_birthday_year)) {
                if (ctype_digit($other_dependent1_birthday_month) && ctype_digit($other_dependent1_birthday_day)) {
                    if (!checkdate($other_dependent1_birthday_month, $other_dependent1_birthday_day, $other_dependent1_birthday_year)) {
                        $validator->errors()->add('other_dependent1_birthday_day', '1枚目_35_生年月日は正しい日付を入力してください。');
                    }
                    if (Carbon::create($other_dependent1_birthday_year,$other_dependent1_birthday_month, $other_dependent1_birthday_day)->gt($today)) {
                        $validator->errors()->add('other_dependent1_birthday_day','1枚目_35_生年月日に未来の日付を入力しないでください。');
                    }
                }
            }
            if ($other_dependent1_birthday_era === '5') {
            ...
            if (!empty($other_dependent1_become_date_month) && !empty($other_dependent1_become_date_day) && !empty($other_dependent1_become_date_year)) {
                if (ctype_digit($other_dependent1_become_date_month) && ctype_digit($other_dependent1_become_date_day)) {
                    if (!checkdate($other_dependent1_become_date_month, $other_dependent1_become_date_day, $other_dependent1_become_date_year)) {
                        $validator->errors()->add('other_dependent1_become_date_day', '1枚目_40_被扶養者になった日は正しい日付を入力してください。');
                    }
                    if (Carbon::create($other_dependent1_become_date_year,$other_dependent1_become_date_month, $other_dependent1_become_date_day)->gt($today)) {
                        $validator->errors()->add('other_dependent1_become_date_day','1枚目_40_被扶養者になった日に未来の日付を入力しないでください。');
                    }
                }
            }
            if($other_dependent1_become_date_era === '7') {
            ...
            if (!empty($other_dependent1_remove_date_month) && !empty($other_dependent1_remove_date_day) && !empty($other_dependent1_remove_date_year)) {
                if (ctype_digit($other_dependent1_remove_date_month) && ctype_digit($other_dependent1_remove_date_day)) {
                    if (!checkdate($other_dependent1_remove_date_month, $other_dependent1_remove_date_day, $other_dependent1_remove_date_year)) {
                        $validator->errors()->add('other_dependent1_remove_date_day', '1枚目_44_被扶養者でなくなった日は正しい日付を入力してください。');
                    }
                    if (Carbon::create($other_dependent1_remove_date_year,$other_dependent1_remove_date_month, $other_dependent1_remove_date_day)->gt($today)) {
                        $validator->errors()->add('other_dependent1_remove_date_day','1枚目_44_被扶養者でなくなった日に未来の日付を入力しないでください。');
                    }
                }
            }
            if($other_dependent1_remove_date_era === '7') {
            ...
            if (!empty($other_dependent1_become_date_era) && !empty($other_dependent1_become_date_year) && !empty($other_dependent1_become_date_month) && !empty($other_dependent1_become_date_day)
            && !empty($other_dependent1_remove_date_era) && !empty($other_dependent1_remove_date_year) && !empty($other_dependent1_remove_date_month) && !empty($other_dependent1_remove_date_day)) {
                if ($other_dependent1_remove_date_year < $other_dependent1_become_date_year) {
                    $validator->errors()->add('other_dependent1_remove_date_day', '1枚目_40_被扶養者でなくなった日は1枚目_44_被扶養者になった日以降を入力してください。');
                } elseif ($other_dependent1_remove_date_year == $other_dependent1_become_date_year) {
                    if ($other_dependent1_remove_date_month < $other_dependent1_become_date_month) {
                        $validator->errors()->add('other_dependent1_remove_date_day', '1枚目_40_被扶養者でなくなった日は1枚目_44_被扶養者になった日以降を入力してください。');
                    } elseif ($other_dependent1_remove_date_month == $other_dependent1_become_date_month) {
                        if ($other_dependent1_remove_date_day < $other_dependent1_become_date_day) {
                            $validator->errors()->add('other_dependent1_remove_date_day', '1枚目_40_被扶養者でなくなった日は1枚目_44_被扶養者になった日以降を入力してください。');
                        }
                    }
                }
            ...
            if (!empty($other_dependent1_domestic_transfer_date_month) && !empty($other_dependent1_domestic_transfer_date_day) && !empty($other_dependent1_domestic_transfer_date_year)) {
                if (ctype_digit($other_dependent1_domestic_transfer_date_month) && ctype_digit($other_dependent1_domestic_transfer_date_day)) {
                    if (!checkdate($other_dependent1_domestic_transfer_date_month, $other_dependent1_domestic_transfer_date_day, $other_dependent1_domestic_transfer_date_year)) {
                        $validator->errors()->add('other_dependent1_domestic_transfer_date_day', '1枚目_50_国内転入日は正しい日付を入力してください。');
                    }
                    if (Carbon::create($other_dependent1_domestic_transfer_date_year,$other_dependent1_domestic_transfer_date_month, $other_dependent1_domestic_transfer_date_day)->gt($today)) {
                        $validator->errors()->add('other_dependent1_domestic_transfer_date_day','1枚目_50_国内転入日に未来の日付を入力しないでください。');
                    }
                }
            }
            ...
            if (!empty($other_dependent2_birthday_month) && !empty($other_dependent2_birthday_day) && !empty($other_dependent2_birthday_year)) {
                if (ctype_digit($other_dependent2_birthday_month) && ctype_digit($other_dependent2_birthday_day)) {
                    if (!checkdate($other_dependent2_birthday_month, $other_dependent2_birthday_day, $other_dependent2_birthday_year)) {
                        $validator->errors()->add('other_dependent2_birthday_day', '1枚目_52_生年月日は正しい日付を入力してください。');
                    }
                    if (Carbon::create($other_dependent2_birthday_year,$other_dependent2_birthday_month, $other_dependent2_birthday_day)->gt($today)) {
                        $validator->errors()->add('other_dependent2_birthday_day','1枚目_52_生年月日に未来の日付を入力しないでください。');
                    }
                }
            }
            if ($other_dependent2_birthday_era === '5') {
            ...
            if (!empty($other_dependent2_become_date_month) && !empty($other_dependent2_become_date_day) && !empty($other_dependent2_become_date_year)) {
                if (ctype_digit($other_dependent2_become_date_month) && ctype_digit($other_dependent2_become_date_day)) {
                    if (!checkdate($other_dependent2_become_date_month, $other_dependent2_become_date_day, $other_dependent2_become_date_year)) {
                        $validator->errors()->add('other_dependent2_become_date_day', '1枚目_57_被扶養者になった日は正しい日付を入力してください。');
                    }
                    if (Carbon::create($other_dependent2_become_date_year,$other_dependent2_become_date_month, $other_dependent2_become_date_day)->gt($today)) {
                        $validator->errors()->add('other_dependent2_become_date_day','1枚目_57_被扶養者になった日に未来の日付を入力しないでください。');
                    }
                }
            }
            if($other_dependent2_become_date_era === '7') {
            ...
            if (!empty($other_dependent2_remove_date_month) && !empty($other_dependent2_remove_date_day) && !empty($other_dependent2_remove_date_year)) {
                if (ctype_digit($other_dependent2_remove_date_month) && ctype_digit($other_dependent2_remove_date_day)) {
                    if (!checkdate($other_dependent2_remove_date_month, $other_dependent2_remove_date_day, $other_dependent2_remove_date_year)) {
                        $validator->errors()->add('other_dependent2_remove_date_day', '1枚目_61_被扶養者でなくなった日は正しい日付を入力してください。');
                    }
                    if (Carbon::create($other_dependent2_remove_date_year,$other_dependent2_remove_date_month, $other_dependent2_remove_date_day)->gt($today)) {
                        $validator->errors()->add('other_dependent2_remove_date_day','1枚目_61_被扶養者でなくなった日に未来の日付を入力しないでください。');
                    }
                }
            }
            if($other_dependent2_remove_date_era === '7') {
            ...
            if (!empty($other_dependent2_become_date_era) && !empty($other_dependent2_become_date_year) && !empty($other_dependent2_become_date_month) && !empty($other_dependent2_become_date_day)
            && !empty($other_dependent2_remove_date_era) && !empty($other_dependent2_remove_date_year) && !empty($other_dependent2_remove_date_month) && !empty($other_dependent2_remove_date_day)) {
                if ($other_dependent2_remove_date_year < $other_dependent2_become_date_year) {
                    $validator->errors()->add('other_dependent2_remove_date_day', '1枚目_57_被扶養者でなくなった日は1枚目_61_被扶養者になった日以降を入力してください。');
                } elseif ($other_dependent2_remove_date_year == $other_dependent2_become_date_year) {
                    if ($other_dependent2_remove_date_month < $other_dependent2_become_date_month) {
                        $validator->errors()->add('other_dependent2_remove_date_day', '1枚目_57_被扶養者でなくなった日は1枚目_61_被扶養者になった日以降を入力してください。');
                    } elseif ($other_dependent2_remove_date_month == $other_dependent2_become_date_month) {
                        if ($other_dependent2_remove_date_day < $other_dependent2_become_date_day) {
                            $validator->errors()->add('other_dependent2_remove_date_day', '1枚目_57_被扶養者でなくなった日は1枚目_61_被扶養者になった日以降を入力してください。');
                        }
                    }
                }
            ...
            if (!empty($other_dependent2_domestic_transfer_date_month) && !empty($other_dependent2_domestic_transfer_date_day) && !empty($other_dependent2_domestic_transfer_date_year)) {
                if (ctype_digit($other_dependent2_domestic_transfer_date_month)) {
                    if (!checkdate($other_dependent2_domestic_transfer_date_month, $other_dependent2_domestic_transfer_date_day, $other_dependent2_domestic_transfer_date_year)) {
                        $validator->errors()->add('other_dependent2_domestic_transfer_date_day', '1枚目_67_国内転入日は正しい日付を入力してください。');
                    }
                    if (Carbon::create($other_dependent2_domestic_transfer_date_year,$other_dependent2_domestic_transfer_date_month, $other_dependent2_domestic_transfer_date_day)->gt($today)) {
                        $validator->errors()->add('other_dependent2_domestic_transfer_date_day','1枚目_67_国内転入日に未来の日付を入力しないでください。');
                    }
                }
            }
    ...
    public function messages()
    {
        return [
            ...
            'spouse_become_date_era' => '1枚目_21_被扶養者になった日_年号は平成,令和のいずれかである必要があります。',
            'spouse_remove_date_era' => '1枚目_25_被扶養者でなくなった日_年号は平成,令和のいずれかである必要があります。',
            ...
            'other_dependent1_birthday_era' => '1枚目_35_生年月日_年号は昭和,平成,令和のいずれかである必要があります。',
            'other_dependent1_sex' => '1枚目_35_性別は男,女のいずれかである必要があります。',
            'other_dependent1_become_date_era' => '1枚目_40_被扶養者になった日_年号は平成,令和のいずれかである必要があります。',
            'other_dependent1_remove_date_era' => '1枚目_44_被扶養者でなくなった日_年号は平成,令和のいずれかである必要があります。',
            ...
            'other_dependent1_special_requirements_non_applicable_reason_type' => '1枚目_50_理由は国内転入,その他のいずれかである必要があります。',
            'other_dependent2_birthday_era' => '1枚目_52_生年月日_年号は昭和,平成,令和のいずれかである必要があります。',
            'other_dependent2_sex' => '1枚目_50_性別は男,女のいずれかである必要があります。',
            'other_dependent2_remove_date_era' => '1枚目_61_被扶養者でなくなった日_年号は平成,令和のいずれかである必要があります。',
            ...
            'other_dependent2_special_requirements_non_applicable_reason_type' => '1枚目_67_理由は国内転入,その他のいずれかである必要があります。',
            ...
            'spouse_become_date_era.required_with' => '1枚目_21_被扶養者になった日_年号を入力してください。',
            'spouse_become_date_year.required_with' => '1枚目_21_被扶養者になった日_年を入力してください。',
            'spouse_become_date_month.required_with' => '1枚目_21_被扶養者になった日_月を入力してください。',
            'spouse_become_date_day.required_with' => '1枚目_21_被扶養者になった日_日を入力してください。',
            'spouse_remove_date_era.required_with' => '1枚目_25_被扶養者でなくなった日_年号を入力してください。',
            'spouse_remove_date_year.required_with' => '1枚目_25_被扶養者でなくなった日_年を入力してください。',
            'spouse_remove_date_month.required_with' => '1枚目_25_被扶養者でなくなった日_月を入力してください。',
            'spouse_remove_date_day.required_with' => '1枚目_25_被扶養者でなくなった日_日を入力してください。',
            'spouse_passed_away_date_year.required_with' => '1枚目_26_理由_死亡年月日_年を入力してください。',
            'spouse_passed_away_date_month.required_with' => '1枚目_26_理由_死亡年月日_月を入力してください。',
            'spouse_passed_away_date_day.required_with' => '1枚目_26_理由_死亡年月日_日を入力してください。',
            ...
            'other_dependent1_birthday_era.required_with' => '1枚目_35_生年月日_年号を入力してください。',
            'other_dependent1_birthday_year.required_with' => '1枚目_35_生年月日_年を入力してください。',
            'other_dependent1_birthday_month.required_with' => '1枚目_35_生年月日_月を入力してください。',
            'other_dependent1_birthday_day.required_with' => '1枚目_35_生年月日_日を入力してください。',
            'other_dependent1_become_date_era.required_with' => '1枚目_40_被扶養者になった日1枚目_44_被扶養者でなくなった日_年号を入力してください。',
            'other_dependent1_become_date_year.required_with' => '1枚目_40_被扶養者になった日1枚目_44_被扶養者でなくなった日_年を入力してください。',
            'other_dependent1_become_date_month.required_with' => '1枚目_40_被扶養者になった日1枚目_44_被扶養者でなくなった日_月を入力してください。',
            'other_dependent1_become_date_day.required_with' => '1枚目_40_被扶養者になった日1枚目_44_被扶養者でなくなった日_日を入力してください。',
            'other_dependent1_remove_date_era.required_with' => '1枚目_44_被扶養者でなくなった日_年号を入力してください。',
            'other_dependent1_remove_date_year.required_with' => '1枚目_44_被扶養者でなくなった日_年を入力してください。',
            'other_dependent1_remove_date_month.required_with' => '1枚目_44_被扶養者でなくなった日_月を入力してください。',
            'other_dependent1_remove_date_day.required_with' => '1枚目_44_被扶養者でなくなった日_日を入力してください。',
            'other_dependent1_domestic_transfer_date_year.required_with' => '1枚目_50_国内転入日1枚目_52_生年月日_年を入力してください。',
            'other_dependent1_domestic_transfer_date_month.required_with' => '1枚目_50_国内転入日1枚目_52_生年月日_月を入力してください。',
            'other_dependent1_domestic_transfer_date_day.required_with' => '1枚目_50_国内転入日1枚目_52_生年月日_日を入力してください。',
            'other_dependent2_birthday_era.required_with' => '1枚目_52_生年月日_年号を入力してください。',
            'other_dependent2_birthday_year.required_with' => '1枚目_52_生年月日_年を入力してください。',
            'other_dependent2_birthday_month.required_with' => '1枚目_52_生年月日_月を入力してください。',
            'other_dependent2_birthday_day.required_with' => '1枚目_52_生年月日_日を入力してください。',
            'other_dependent2_become_date_era.required_with' => '1枚目_57_被扶養者になった日_年号を入力してください。',
            'other_dependent2_become_date_year.required_with' => '1枚目_57_被扶養者になった日_年を入力してください。',
            'other_dependent2_become_date_month.required_with' => '1枚目_57_被扶養者になった日_月を入力してください。',
            'other_dependent2_become_date_day.required_with' => '1枚目_57_被扶養者になった日_日を入力してください。',
            'other_dependent2_remove_date_era.required_with' => '1枚目_61_被扶養者でなくなった日_年号を入力してください。',
            'other_dependent2_remove_date_year.required_with' => '1枚目_61_被扶養者でなくなった日_年を入力してください。',
            'other_dependent2_remove_date_month.required_with' => '1枚目_61_被扶養者でなくなった日_月を入力してください。',
            'other_dependent2_remove_date_day.required_with' => '1枚目_61_被扶養者でなくなった日_日を入力してください。',
            'other_dependent2_domestic_transfer_date_year.required_with' => '1枚目_67_国内転入日_年を入力してください。',
            'other_dependent2_domestic_transfer_date_month.required_with' => '1枚目_67_国内転入日_月を入力してください。',
            'other_dependent2_domestic_transfer_date_day.required_with' => '1枚目_67_国内転入日_日を入力してください。',
        ];
    }
    public function attributes()
    {
        return [
            ...
            'spouse_become_date_era' => '1枚目_21_被扶養者になった日_年号',
            'spouse_become_date_year' => '1枚目_21_被扶養者になった日_年',
            'spouse_become_date_month' => '1枚目_21_被扶養者になった日_月',
            'spouse_become_date_day' => '1枚目_21_被扶養者になった日_日',
            'spouse_reason_type_2' => '1枚目_22_理由',
            'spouse_reason_2' => '1枚目_22_理由_その他入力欄',
            'spouse_reason_type' => '1枚目_26_理由',
            'spouse_remove_date_era' => '1枚目_25_被扶養者でなくなった日_年号',
            'spouse_remove_date_year' => '1枚目_25_被扶養者でなくなった日_年',
            'spouse_remove_date_month' => '1枚目_25_被扶養者でなくなった日_月',
            'spouse_remove_date_day' => '1枚目_25_被扶養者でなくなった日_日',
            'spouse_passed_away_date_year' => '1枚目_26_理由_死亡年月日_年',
            'spouse_passed_away_date_month' => '1枚目_26_理由_死亡年月日_月',
            'spouse_passed_away_date_day' => '1枚目_26_理由_死亡年月日_日',
            'spouse_reason' => '1枚目_26_理由_その他入力欄',
            'spouse_occupation_type' => '1枚目_23_職業',
            'spouse_occupation' => '1枚目_23_職業_その他入力欄',
            'dependent_annual_income' => '1枚目_24_収入（年収）',
            ...
            'eligibility_confirmation_letter_1' => '1枚目_33_資格確認書発行要否',
            'other_dependent1_name' => '1枚目_34_氏名',
            'other_dependent1_name_kana' => '1枚目_34_氏名（フリガナ）',
            'other_dependent1_birthday_era' => '1枚目_35_生年月日_年号',
            'other_dependent1_birthday_year' => '1枚目_35_生年月日_年',
            'other_dependent1_birthday_month' => '1枚目_35_生年月日_月',
            'other_dependent1_birthday_day' => '1枚目_35_生年月日_日',
            'other_dependent1_sex' => '1枚目_36_性別',
            'other_dependent1_relationship' => '1枚目_37_続柄',
            'other_dependent1_mynumber_card_no' => '1枚目_38_個人番号',
            'other_dependent1_living_type' => '1枚目_39_住所_共同居住状況',
            'other_dependent1_post_code_former' => '1枚目_39_住所_郵便番号3桁',
            'other_dependent1_post_code_latter' => '1枚目_39_住所_郵便番号4桁',
            'other_dependent1_address' => '1枚目_39_住所_所在地',
            'other_dependent1_become_date_era' => '1枚目_40_被扶養者になった日_年号',
            'other_dependent1_become_date_year' => '1枚目_40_被扶養者になった日_年',
            'other_dependent1_become_date_month' => '1枚目_40_被扶養者になった日_月',
            'other_dependent1_become_date_day' => '1枚目_40_被扶養者になった日_日',
            'other_dependent1_reason_type' => '1枚目_41_理由',
            'other_dependent1_remove_date_era' => '1枚目_44_被扶養者でなくなった日_年号',
            'other_dependent1_remove_date_year' => '1枚目_44_被扶養者でなくなった日_年',
            'other_dependent1_remove_date_month' => '1枚目_44_被扶養者でなくなった日_月',
            'other_dependent1_remove_date_day' => '1枚目_44_被扶養者でなくなった日_日',
            'other_dependent1_reason_type_2' => '1枚目_45_理由',
            'other_dependent1_reason_2' => '1枚目_45_理由_その他入力欄',
            'other_dependent1_reason' => '1枚目_41_理由_その他入力欄',
            'other_dependent1_occupation_type' => '1枚目_42_職業',
            'other_dependent1_occupation' => '1枚目_42_職業_その他入力欄',
            'other_dependent1_occupation_type_grade' => '1枚目_42_職業_高・大学生の場合の学年',
            'other_dependent1_annual_income' => '1枚目_43_収入（年収）',
            'other_dependent1_special_requirements_applicable_flg' => '1枚目_48_海外特例要件',
            'other_dependent1_special_requirements_applicable_reason_type' => '1枚目_49_理由',
            'other_dependent1_special_requirements_applicable_reason' => '1枚目_49_理由_その他入力欄',
            'other_dependent1_special_requirements_non_applicable_reason_type' => '1枚目_50_理由',
            'other_dependent1_domestic_transfer_date_year' => '1枚目_50_国内転入日_年',
            'other_dependent1_domestic_transfer_date_month' => '1枚目_50_国内転入日_月',
            'other_dependent1_domestic_transfer_date_day' => '1枚目_50_国内転入日_日',
            'other_dependent1_special_requirements_non_applicable_reason' => '1枚目_50_理由_その他入力欄',
            'other_dependent1_remarks' => '1枚目_46_備考',
            'other_dependent1_confirmation_relationship_0' => '1枚目_46_備考_続柄確認済みチェックボックス',
            'eligibility_confirmation_letter_2' => '1枚目_47_発行確認書発行要否',
            'other_dependent2_name' => '1枚目_51_氏名',
            'other_dependent2_name_kana' => '1枚目_51_氏名（フリガナ）',
            'other_dependent2_birthday_era' => '1枚目_52_生年月日_年号',
            'other_dependent2_birthday_year' => '1枚目_52_生年月日_年',
            'other_dependent2_birthday_month' => '1枚目_52_生年月日_月',
            'other_dependent2_birthday_day' => '1枚目_52_生年月日_日',
            'other_dependent2_sex' => '1枚目_53_性別',
            'other_dependent2_relationship' => '1枚目_54_続柄',
            'other_dependent2_mynumber_card_no' => '1枚目_55_個人番号',
            'other_dependent2_living_type' => '1枚目_56_住所_共同居住状況',
            'other_dependent2_post_code_former' => '1枚目_56_住所_郵便番号3桁',
            'other_dependent2_post_code_latter' => '1枚目_56_住所_郵便番号4桁',
            'other_dependent2_address' => '1枚目_56_住所_所在地',
            'other_dependent2_become_date_era' => '1枚目_57_被扶養者になった日_年号',
            'other_dependent2_become_date_year' => '1枚目_57_被扶養者になった日_年',
            'other_dependent2_become_date_month' => '1枚目_57_被扶養者になった日_月',
            'other_dependent2_become_date_day' => '1枚目_57_被扶養者になった日_日',
            'other_dependent2_reason_type' => '1枚目_58_理由',
            'other_dependent2_remove_date_era' => '1枚目_61_被扶養者でなくなった日_年号',
            'other_dependent2_remove_date_year' => '1枚目_61_被扶養者でなくなった日_年',
            'other_dependent2_remove_date_month' => '1枚目_61_被扶養者でなくなった日_月',
            'other_dependent2_remove_date_day' => '1枚目_61_被扶養者でなくなった日_日',
            'other_dependent2_reason_type_2' => '1枚目_62_理由',
            'other_dependent2_reason' => '1枚目_58_理由_その他入力欄',
            'other_dependent2_occupation_type' => '1枚目_59_職業',
            'other_dependent2_occupation' => '1枚目_59_職業_その他入力欄',
            'other_dependent2_occupation_type_grade' => '1枚目_59_職業_高・大学生の場合の学年',
            'other_dependent2_annual_income' => '1枚目_60_収入（年収）',
            'other_dependent2_special_requirements_applicable_flg' => '1枚目_65_海外特例要件',
            'other_dependent2_special_requirements_applicable_reason_type' => '1枚目_66理由',
            'other_dependent2_special_requirements_applicable_reason' => '1枚目_66_理由_その他入力欄',
            'other_dependent2_special_requirements_non_applicable_reason_type' => '1枚目_67_理由',
            'other_dependent2_domestic_transfer_date_year' => '1枚目_67_国内転入日_年',
            'other_dependent2_domestic_transfer_date_month' => '1枚目_67_国内転入日_月',
            'other_dependent2_domestic_transfer_date_day' => '1枚目_67_国内転入日_日',
            'other_dependent2_special_requirements_non_applicable_reason' => '1枚目_67_理由_その他入力欄',
            'other_dependent2_remarks' => '1枚目_63_備考',
            'other_dependent2_confirmation_relationship_0' => '1枚目_63_備考_続柄確認済みチェックボックス',
            'eligibility_confirmation_letter_3' => '1枚目_64_資格確認書発行要否',
