...
class FirstParentalLeaveBenefitsForEmploymentInsuranceRequest extends BaseRequest
{
    ...
    public function validationData()
    {
        ...
        if (isset($data['financial_Institutions_name_kana'])) {
            $data['financial_Institutions_name_kana'] = mb_convert_kana($data['financial_Institutions_name_kana'], 'KS');
        }
        if (isset($data['financial_institution_name'])) {
            $data['financial_institution_name'] = mb_convert_kana($data['financial_institution_name'], 'AKS');
        }
        if (isset($data['address_prefecture_city'])) {
            $data['address_prefecture_city'] = mb_convert_kana($data['address_prefecture_city'], 'KS');
            $data['address_prefecture_city'] = str_replace(['-', '‐', '―'], '－', $data['address_prefecture_city']);
        }
        if (isset($data['address_ward'])) {
            $data['address_ward'] = mb_convert_kana($data['address_ward'], 'AKS');
            $data['address_ward'] = str_replace(['-', '‐', '―'], '－', $data['address_ward']);
        }
        if (isset($data['address_apartment'])) {
            $data['address_apartment'] = mb_convert_kana($data['address_apartment'], 'AKS');
