class CsvImportAttendance extends PowerTableList {
    constructor(options) {
        super(options)
    ...
    onCreateCell(parent, key, item) {
        if (key == 'working_days') {
            ...
            label.textContent = this.replaceInt(item.working_normal_days) + this.replaceInt(item.working_off_days) + this.replaceInt(item.working_legal_days);
