class WageFilter extends PowerTableFilter {
    ...
    setFilterConfig(j) {
        ...
        wage_department.forEach(select => {
            for (let i = 0; i < select.children.length; i++) {
                const option = select.children[i];
                if (option.value == json.wage_department) {
                    option.dataset.n = 0;
                    option.selected = true;
                }
            }
        ...
        gradational_salary.forEach(select => {
            for (let i = 0; i < select.children.length; i++) {
                const option = select.children[i];
                if (option.value == json.gradational_salary) {
                    option.dataset.n = 0;
                    option.selected = true;
                }
            }
        ...
        other_type.forEach(select => {
            for (let i = 0; i < select.children.length; i++) {
                const option = select.children[i];
                if (option.value == json.other_type) {
                    option.dataset.n = 0;
                    option.selected = true;
                }
            }
        ...
        json.conditions.forEach(cond => {
            const row = this.createConditionNode();
    ...
    setInsurances(insurances) {
        this.PowerList.labor_insurances = insurances.labor;
    ...
    onAdd() {
        if (this.filterConditionNodes.length > 4) return;
        const node = this.createConditionNode();
