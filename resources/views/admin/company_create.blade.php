<x-layout title="{{ !isset($company_id) ? '会社情報登録' : '会社情報編集' }}" useRightContent="{{ false }}">
                            ...
                            <div class="required field {{ err($errors, 'name') }}">
                                ...
                                @if (!isset($company_id))
                                    <input type="text" id="name" name="name" value="{{ old('name') }}"
                                        placeholder="株式会社Karte" autocomplete="off">
                                @else
                            ...
                            <div class="required field {{ err($errors, 'name_kana') }}">
                                ...
                                @if (!isset($company_id))
                                    <input type="text" id="name_kana" name="name_kana"
                                        value="{{ old('name_kana') }}" placeholder="カブシキガイシャカルテ" autocomplete="off">
                                @else
                                    <input type="text" id="name_kana" name="name_kana"
                            ...
                            <div class="ui toggle checkbox">
                                @if (!isset($company_id))
                                    ...
                                    <input type="checkbox" name="procedure_hidden_flg" value="1"
                                        {{ old('procedure_hidden_flg') == '1' ? 'checked' : '' }} autocomplete="off">
                                @else
                                    <input type="hidden" name="procedure_hidden_flg" value="0">
                                    <input type="checkbox" name="procedure_hidden_flg" value="1"
                                        {{ $company->procedure_hidden_flg == '1' ? 'checked' : '' }} autocomplete="off">
                                @endif
    ...
    <!-- 業種選択モーダル -->
    <x-search-industry-type-modal id="industry_type_select" selectorId="{{ $company_id ?? '' }}" />
    <script type="module">
            ...
            $.ajax({
                ...
                .done((data) => {
                    ...
                    if (first) {
                        ...
                        def.forEach(v => {
                            let a = $('select[name="industry_type[]"] option[value=' + v +
                                ']').prop(
                                'selected', true);
                        });
                    }
