...
<section class="content">
    ...
    @endslot
    <h1>帳票：介護休業給付（介護休業給付金）の申請 / 雇用保険介護休業給付（介護休業給付金）の申請</h1>
    ...
    <script type="module">
        $(document).ready(function () {
            ...
            // 申請年月日
            $('#J80_005F_944E').val('{{$today["year"]}}');
            $('#J81_005F_8C8E').val('{{$today["month"]}}');
            $('#J82_005F_93FA').val('{{$today["day"]}}');

            // 社会保険労務士記載欄
            if ({{$current_employee->role_id}} === 500) {
                // 帳票1枚目
                // 提出代行者・事務代理者
                $('#J113_005F_8DEC_90AC_944E_8C8E_93FA_005F_92F1_8F6F_91E3_8D73_8ED2').val('{{$current_employee->last_name ?: ''}}' + '　' + '{{$current_employee->first_name ?: ''}}');
                // 氏名
                $('#J114_005F_8E81_96BC').val('{{$current_employee->last_name ?: ''}}' + '　' + '{{$current_employee->first_name ?: ''}}');

                // 作成年月日
                $('#J75_005F_944E').val('{{$today["year"]}}');
                $('#J76_005F_8C8E').val('{{$today["month"]}}');
                $('#J77_005F_93FA').val('{{$today["day"]}}');
                // 提出代行者・事務代理者
