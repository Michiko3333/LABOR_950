    ...
    window.bonus = () => {
            ...
            function getDepartmentList(id, first = false) {
                $.ajax({
                    ...
                    .done((data) => {
                        $('select[name="bo-departments[]"]').empty();
                        data.forEach(element => {
                            $('<option>').attr({
                                value: element.id
                            }).text(element.name).appendTo('select[name="bo-departments[]"]');
                        });
                        ...
                        if (first) {
                            ...
                            def.forEach(v => {
                                let a = $('select[name="bo-departments[]"] option[value=' + v +
                                    ']').prop(
                                    'selected', true);
                            });
                        }
    ...
    <input type="hidden" name="bo-id[{{ $childKey }}][]" value="{{ $bonusItem['bo-id'] }}" />
        <div class="six wide field {{ err_sub($boErrs, 'bo-departments', $childKey, $bonusKey) }}" wire:ignore>
            ...
            <select class="ui fluid search dropdown multiple department_select"
                wire:model.live="bonusData.{{ $bonusKey }}.bo-departments"
                multiple="" name="bo-departments[{{ $childKey }}][{{ $bonusKey }}][]">
                @foreach($departments as $department)
                    <option value="{{ $department->id }}">{{ $department->name }}</option>
                @endforeach
            </select>
        </div>
        <div class="six wide field {{ err_sub($boErrs, 'bo-bonus_payment_month', $childKey, $bonusKey) }}" wire:ignore>
            ...
            <div class="ui calendar month-calendar" wire:ignore>
                <div class="ui fluid input left icon">
                    ...
                    <input type="text" name="bo-applied_date[{{ $childKey }}][]"
                        wire:model.live="bonusData.{{ $bonusKey }}.bo-applied_date"
                        placeholder="YYYY年M月">
                </div>
            </div>
        </div>
        ...
        <div class="field">
            <label></label>
            <button class="ui button icon basic negative button-disable" type="button" wire:click="removebonus({{ $bonusKey }})"><i
...
@endforeach
@if ($userPermission->isBasicDepartment() && $userPermission->isWritableFor(2))
        <button class="append-bonus button-disable" type="button" wire:click="bonusAppend" id="append-bonus_{{ $childKey }}"
            {{ count($bonusData) > 9 ? 'disabled' : '' }}><i class="plus circle icon"></i>追加</button>
@endif
@script
    <script type="module">
        const notReadonly = @json($userPermission->isBasicDepartment() && $userPermission->isWritableFor(2));
        $(document).ready(function() {
            if (notReadonly) bonus();
        });
        $(document).off('click', '.bonus-history-{{ $childKey }}').on('click', '.bonus-history-{{ $childKey }}', function() {
