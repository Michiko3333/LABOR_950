<div class="branch-container">
            ...
            $('.ui.accordion').accordion({
                onChange: function(e) {
                    ...
                    for (let i = 0; i < acs.length; i++) {
                        ...
                        d.push({index: k, value: cls});
                    }

                    Livewire.dispatch('active-state', { data: d });
                },
            });
        }
    </script>
            ...
            <div class="content_inner">
                <div class="two fields" style="padding: 0;">
                    <div class="field required {{err_bind($errs, 'br-name', $key)}}">
                        ...
                        <input type="text" name="br-name[]" wire:model.live="data.{{ $key }}.br-name" placeholder="">
                    </div>
                    <div class="ui unstackable two fields">
                        <div class="field required {{err_bind($errs, 'br-branch_type', $key)}}">
                            ...
                            <select class="ui fluid dropdown" name="br-branch_type[]"
                                wire:model.live="data.{{ $key }}.br-branch_type" {{$key==0 ? 'disabled' : '' }}>
                                @foreach ($branch_types as $k => $value)
                                @if($key > 0 && $k == 1)
                                @continue
                                @endif
                                <option value="{{$k}}" {{$key==0 ? 'selected' : '' }}>{{$value}}</option>
                                @endforeach
                            </select>
                            @if($key==0) <input type="hidden" name="br-branch_type[]" value="1" /> @endif
                        </div>
                ...
                <div class="fields">
                    <div class="four wide field required {{err_bind($errs, 'br-post_code', $key)}}">
                        ...
                        <input type="text" name="br-post_code[]" wire:model.live="data.{{ $key }}.br-post_code"
                            placeholder="">
                    </div>
                    ...
                    <div class="eight wide field required {{err_bind($errs, 'br-address_city', $key)}}">
                        ...
                        <input type="text" name="br-address_city[]" wire:model.live="data.{{ $key }}.br-address_city"
                            placeholder="">
                    </div>
                </div>
                <div class="two fields">
                    <div class="field required {{err_bind($errs, 'br-address_ward', $key)}}">
                        ...
                        <input type="text" name="br-address_ward[]" wire:model.live="data.{{ $key }}.br-address_ward"
                            placeholder="">
                    </div>
                    ...
                    <div class="ui unstackable three fields field" style="padding: 0;">
                        ...
                        <div class="field {{err_bind($errs, 'br-tel_subscriber_code', $key)}}"
                            ...
                            <input type="text" name="br-tel_subscriber_code[]"
                                wire:model.live="data.{{ $key }}.br-tel_subscriber_code" placeholder="加入者番号">
                        </div>
                    </div>
                    <div class="field {{err_bind($errs, 'br-tel_overseas', $key)}}">
                        <label for="br-tel_overseas">国外電話番号</label>
                        <input type="text" name="br-tel_overseas[]" wire:model.live="data.{{ $key }}.br-tel_overseas"
                            placeholder="">
                    </div>
                </div>
                <div class="ui divider my-2"></div>
                <div class="three fields">
                    ...
                    <div class="field {{err_bind($errs, 'br-labor_insurance_payment_method', $key)}}">
                        ...
                        <select class="ui fluid dropdown" name="br-labor_insurance_payment_method[]"
                            ...
                            <option value="">未選択</option>
                            <option value="1">口座振替</option>
                            <option value="2">窓口納付</option>
                        </select>
                    </div>

                    <div class="field {{err_bind($errs, 'br-insurance_type_id', $key)}}">
                        <label for="br-insurance_type_id">労働保険種類の分類</label>
                        <select class="ui fluid dropdown" name="br-insurance_type_id[]"
                            wire:model.live="data.{{ $key }}.br-insurance_type_id">
                            <option value="">未選択</option>
                            @foreach ($insuranceTypes as $k => $value)
                            <option value="{{$k}}">{{$value}}</option>
                            @endforeach
                        </select>
                    </div>
            ...
            <div class="content_inner">
                ...
                <div class="three fields">
                    <div class="field {{err_bind($errs, 'br-start_date_of_month', $key)}}">
                        <label for="br-start_date_of_month">開始設定(月の始まり)</label>
                        <input type="text" name="br-start_date_of_month[]" placeholder="">
                    </div>

                    <div class="field {{err_bind($errs, 'br-start_days_of_week', $key)}}">
                        <label for="br-start_days_of_week">開始設定(週の始まり)</label>
                        <input type="text" name="br-start_days_of_week[]" placeholder="">
                    </div>

                    <div class="field {{err_bind($errs, 'br-start_time_of_day', $key)}}">
                        <label for="br-start_time_of_day">開始設定(日の始まり)</label>
                ...
                <div class="two fields">
                    ...
                    <div class="field {{err_bind($errs, 'br-work_time_end', $key)}}">
                        <label for="br-work_time_end">就業時間(終了)</label>
                ...
                <div class="three fields">
                    <div class="field {{err_bind($errs, 'br-holiday_yearly', $key)}}">
                        <label for="br-holiday_yearly">休日日数(年間)</label>
    ...
    @endforeach
    <button class="append-branch" type="button" wire:click="append" {{count($data)> 9 ?
        'disabled' : ''}}><i class="plus circle icon"></i>事業所を追加</button>
    @script
    <script type="module">
