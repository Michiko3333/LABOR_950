    ...
    window.bounty = () => {
            ...
            function getDepartmentList(id, first = false) {
                $.ajax({
                    ...
                    .done((data) => {
                        $('select[name="bou-departments[]"]').empty();
                        data.forEach(element => {
                            $('<option>').attr({
                                value: element.id
                            }).text(element.name).appendTo('select[name="bou-departments[]"]');
                        });
                        ...
                        if (first) {
                            ...
                            def.forEach(v => {
                                let a = $('select[name="bou-departments[]"] option[value=' + v +
                                    ']').prop(
                                    'selected', true);
                            });
                        }
    ...
    <input type="hidden" name="bou-id[{{ $childKey }}][]" value="{{ $bountyItem['bou-id'] }}" />
        <div class="six wide field {{ err_sub($bouErrs, 'bou-departments', $childKey, $bountyKey) }}" wire:ignore>
            ...
            <select class="ui fluid search dropdown multiple department_select"
                wire:model.live="bountyData.{{ $bountyKey }}.bou-departments"
                multiple="" name="bou-departments[{{ $childKey }}][{{ $bountyKey }}][]">
                @foreach($departments as $department)
                    <option value="{{ $department->id }}">{{ $department->name }}</option>
                @endforeach
            </select>
        </div>
        <div class="six wide field {{ err_sub($bouErrs, 'bou-bonus_payment_month', $childKey, $bountyKey) }}" wire:ignore>
            <label for="bou-bonus_payment_month">支払月</label>
            <select class="ui fluid multiple dropdown limit-select"
            ...
            <div class="ui calendar month-calendar" wire:ignore>
                <div class="ui fluid input left icon">
                    ...
                    <input type="text" name="bou-applied_date[{{ $childKey }}][]"
                        wire:model.live="bountyData.{{ $bountyKey }}.bou-applied_date"
                        placeholder="YYYY年M月">
                </div>
            </div>
        </div>
        ...
        <div class="field">
            <label></label>
            <button class="ui button icon basic negative button-disable" type="button" wire:click="removebounty({{ $bountyKey }})"><i
...
@endforeach
@if ($userPermission->isBasicDepartment() && $userPermission->isWritableFor(2))
        <button class="append-bounty button-disable" type="button" wire:click="bountyAppend" id="append-bounty_{{ $childKey }}"
            {{ count($bountyData) > 9 ? 'disabled' : '' }}><i class="plus circle icon"></i>追加</button>
@endif
@script
    <script type="module">
        const notReadonly = @json($userPermission->isBasicDepartment() && $userPermission->isWritableFor(2));
        $(document).ready(function() {
            if (notReadonly) bounty();
        });
        $(document).off('click', '.bounty-history-{{ $childKey }}').on('click', '.bounty-history-{{ $childKey }}', function() {
